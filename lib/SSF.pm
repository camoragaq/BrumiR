...
sub new{
  ...
  my ($packagename, $foldobj) = @_;
  #print Dumper($unitigs);
  my $self = { fold => $foldobj};
  bless ($self, $packagename);
  ## infer the following secondary structure types
 # paired "Stem"     S
 # Multiloop         M
 # Internal loop     I
 # Bulge             B
 ...
 # dangling End      E
 # eXternal loop     X
 my ($bp,$seq)=readDotBracket($self->{fold}->{seq},$self->{fold}->{struct});
 ...
 my($segments,$knots,$warnings) = separateSegments($allSegments);
  ...
  #print Dumper($structureTypes);
  #we save the structureTypes
  $self->{structureTypes}=$structureTypes;
  #print Dumper($allSegments);
   #my($segments,$knots,$warnings) = separateSegments($allSegments);
   #my $bp = filterBasePairs($seq,$knots);
   #$segments = getSegments($bp);
   #my($dotbracket,$s,$k,$structureTypes,$pageNumber) = buildStructureMap($segments,$knots,$bp,$seq);
   #print Dumper($structureTypes);
...
sub plain_ssf{
  ...
  #my $sst={S=>"Stem",M=>"Multiloop", I=>"Internal loop",B=>"Bulge", H=>"Hairpin loop",K=>"pseudoKnot",E=>"danglind end",X=>"External loop"};
  #my @stypes= sort keys %{$sst};
  my @stypes=("B","E", "H", "I","K","M", "S","X","SEGMENTS");
  #print join(" ",@stypes);
  #print Dumper($sst);
  #print Dumper($self->{structureTypes});
  my @tmp=();
  foreach my $t(@stypes){
    if(defined $self->{structureTypes}->{$t}){
          #print Dumper($self->{structureTypes}->{$t});
          my $r=$self->{structureTypes}->{$t};
          #print ref($r)."\n";
          if(ref($r) eq "ARRAY" ){
            push(@tmp,scalar(@{$r}));
          }else{
            #print Dumper($r);
            push(@tmp,scalar(keys %{$r}));
          }
         #push(@tmp,scalar(@{$r->{$t}}));
    }else{
      push (@tmp,0);
...
sub getBrackets {
    my($n) = @_;
    die "Fatal error: too many (n>29) PKs to represent in dotbracket! \n" if($n >= 30);
    my $left = "([{<" . join("",("A".."Z"));
    my $right = ")]}>" . join("",("a".."z"));
